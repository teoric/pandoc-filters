...
function convert_measurements(size)
  ...
  if size ~= nil then
    ...
    if
      string.match(size, "^[0-9.]+cm")
      or string.match(size, "^[0-9.]+p")
      or string.match(size, "^[0-9.]+P")
      or string.match(size, "^[0-9.]+in")
    then
      return size
    end
  end
...
return {
  ...
  },
  {
    ...
    Image = function(im)
      ...
      if FORMAT == "ms" then
        pat = keep_pattern
        if im.attributes.float and (text.lower(im.attributes.float) == "true" or im.attributes.float == "1") then
          pat = pat .. " F"
        else
          pat = pat .. " S"
        end
    ...
    SmallCaps = function(elem)
      ...
      if FORMAT == "ms" then
        if use_small_caps == "underline" then
          return List:new({
            pandoc.RawInline("ms", string.format('\\c\n.UL "%s"\\c\n', utils.stringify(elem.c))),
          })
