...
local function convert_to_svg(im)
	...
	if Loc_utils.endswith(text.lower(im.src), ".pdf") or Loc_utils.endswith(text.lower(im.src), ".eps") then
...
local function convert_to_emf(im)
	...
	elseif
		not Loc_utils.endswith(text.lower(im.src), ".emf")
		and not (
			Loc_utils.endswith(text.lower(im.src), ".png")
			or Loc_utils.endswith(text.lower(im.src), ".jpg")
			or Loc_utils.endswith(text.lower(im.src), ".jpeg")
		)
	then
		-- let's try our best
		im.src = string.gsub(im.src, "%.[^.]+$", ".emf")
